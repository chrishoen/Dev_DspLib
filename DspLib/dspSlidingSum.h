...
public:
   ...
   // Set true  when the sum reaches high.
   // Set false when the sum reaches low.
   bool mDetectFlag;
   ...
   void doBeforePut() override
   {
      ...
      if (BaseClass::mFullFlag)
      {
         ...
         mSum -= (SumT)BaseClass::elementAtTail();
      }

      // Add the input value to the sum.
      mSum += (SumT)BaseClass::mInput;

      // Compare the sum.
      mHighFlag = mSum >= mSumHigh;
      mLowFlag = mSum <= mSumLow;
      // Set the detection flag.
      if (mHighFlag) mDetectFlag = true;
      if (mLowFlag) mDetectFlag = false;
   }
};
